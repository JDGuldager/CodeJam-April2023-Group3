...
/*
 * Spawns fish, and finds their spawn position and what direction they need to swim.
 */
...
public class CloudSpawner : MonoBehaviour
{
    ...
    void Start()
    {
        // Calls the function SpawnFish() every (1/spawnrate) seconds. (1/spawnrate) gives a number.
    ...
    {
        // Uses FindSpawnPosition to find a spawn position for the fish
        Vector2 spawnPos = FindSpawnPosition();

        // Instantiates the fishPrefab, at the spawn position found above.
		// Quaternion.identity means it spawns with the rotation i has in the prefab fishPrefab
        GameObject cloud = Instantiate(cloudPrefab, spawnPos, Quaternion.identity);

        // Checks if the fish spawns in the left or right side of the map
        if (spawnPos.x > 0)
        {
            // Makes the fish move from right to left when true, otherwise it moves from left to right.
    ...
    public Vector2 FindSpawnPosition()
    {
        ...
        // Decides if the fish spawns in the left or right side. When x is 1, it spawn on the right. When x i -1 it spawns on the left
        if (x == 0)
        {
            x = 1;
        }
